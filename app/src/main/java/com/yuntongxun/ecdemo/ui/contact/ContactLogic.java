...
public class ContactLogic {
    ...
    public static final String[] CONVER_PHONTO = {"icon_touxiang_persion_gray.png"
            , "icon_touxiang_persion_gray.png"
            , "icon_touxiang_persion_gray.png"
            , "icon_touxiang_persion_gray.png"
            , "icon_touxiang_persion_gray.png"
    };
    ...
    static {
        try {
            if (mDefaultBitmap == null) {
                mDefaultBitmap = DemoUtils.decodeStream(CCPAppManager.getContext().getAssets().open("avatar/icon_touxiang_persion_gray.png"), ResourceHelper.getDensity(null));
            }
        } catch (IOException e) {
        }
    }
     ...
     * @return
     */
    public static Bitmap getPhoto02(String username) {
    ...
    public static ArrayList<ECContacts> converContacts(ArrayList<ECContacts> beas) {
        ...
        for (int i = 0; i < beas.size(); i++) {
            ...
            if (i < 5) {
                ...
                accountBean.setRemark("icon_touxiang_persion_gray.png");
            }
        }
        return beas;
    }
        ...
        try {
            ...
            if (cursor != null) {
                ...
                while (cursor.moveToNext()) {
                    ...
                    pyFormat(cli);
                    // 最后一条记录时添加最后一个
                    if (!TextUtils.isEmpty(cli.getContactid())) {
                        if (cursor.getPosition() == (cursor.getCount() - 1)&&cli.getContactid().length() ==11) {
                            if (TextUtils.isEmpty(cli.getNickname())) {
                                cli.setNickname(cli.getContactid());
                            }
                            cl.add(cli);
...
//							char c = name.charAt(i);
    ...
    public static Bitmap getChatroomPhoto(final String groupid) {
        ...
        if (groupid.toUpperCase().startsWith("G")) {
            return BitmapFactory.decodeResource(CCPAppManager.getContext().getResources(), R.drawable.message_icon_qunzu);
        }
        return mDefaultBitmap;
    }
