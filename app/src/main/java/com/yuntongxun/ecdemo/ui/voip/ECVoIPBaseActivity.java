...
public abstract class ECVoIPBaseActivity extends ECSuperActivity
    ...
    private boolean init() {
        ...
        VoiceMeetingService.getInstance().outgoingCall = mIncomingCall;
            ...
            if (infos != null && infos.length > 0) {
                for (String str : infos) {
                    if (str.startsWith(KEY_TEL)) {
                        ...
                        CCPAppManager.PHONE = DemoUtils.getLastwords(str, "=");
                    }
                }
            }
        }
    ...
    @Override
    public void onViewRelease(ECCallControlUILayout controlPanelView, ImageView view) {
        if (controlPanelView != null) {
            controlPanelView.setControlEnable(false);
        }
...
//        setIntent(sIntent);
    ...
    @Override
    public void finish() {
        ECHandlerHelper.postDelayedRunnOnUI(OnCallFinish, 500);
    }
