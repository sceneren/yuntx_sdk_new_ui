...
public class ChattingActivity extends ECFragmentActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(RestServerDefines.ROBOT.equalsIgnoreCase(recipients)){
			AppPanelControl.setIsRobot(true);
			CCPChattingFooter2.setVoice(false);

		}else {
			if (isChatToSelf(recipients) || isPeerChat(recipients)
					|| TextUtils.equals(recipients, RestServerDefines.FILE_ASSISTANT)) {
				AppPanelControl.setShowVoipCall(false);
			}
//			mChattingFragment.setVoice();
