...
public class GroupMemberCardActivity extends ECSuperActivity implements OnClickListener, GroupCardCallBack{
	...
	protected void showInputCodeDialog(String message,final int buttonId) {
		...
		editText = (EditText) view.findViewById(R.id.sendrequest_content);

		switch (buttonId) {
			case R.id.group_card_nickname:
				editText.setInputType(InputType.TYPE_CLASS_TEXT);
				editText.setFilters(new InputFilter[]{new InputFilter.LengthFilter(16)});
				editText.setText(mGroupNameItem.getContextText());
				editText.setSelection(mGroupNameItem.getContextText().length());
				break;
			case R.id.group_card_tel:
				editText.setInputType(InputType.TYPE_CLASS_PHONE);
				editText.setFilters(new InputFilter[]{new InputFilter.LengthFilter(11)});
				editText.setText(mGroupTelItem.getContextText());
				editText.setSelection(mGroupTelItem.getContextText().length());
				break;
			case R.id.group_card_email:
				editText.setInputType(InputType.TYPE_CLASS_TEXT);
				editText.setFilters(new InputFilter[]{new InputFilter.LengthFilter(30)});
				editText.setText(mGroupEmailItem.getContextText());
				editText.setSelection(mGroupEmailItem.getContextText().length());
				break;
			case R.id.group_card_remark:
				editText.setInputType(InputType.TYPE_CLASS_TEXT);
				editText.setFilters(new InputFilter[]{new InputFilter.LengthFilter(56)});
				editText.setText(mGroupRemarkItem.getContextText());
				editText.setSelection(mGroupRemarkItem.getContextText().length());
